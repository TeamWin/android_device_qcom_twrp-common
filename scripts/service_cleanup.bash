...
if [ "$sdkver" -lt 29 ]; then
	...
	included_blobs=($(find "$venbin" -type f -exec echo '{}' \; | awk -F'/' '{print $NF}' | grep keymaster))
else
	...
	included_blobs=($(find "$sysbin" -type f -exec echo '{}' \; | awk -F'/' '{print $NF}' | grep keymaster))
fi
echo "Blobs parsed:"
printf '%s\n' "${included_blobs[@]}"
echo " "
...
rc_service_paths=($(grep "service " "$decrypt_rc" | awk -F'/' '{print $NF}' | grep keymaster))
	...
	case ${service_name[@]} in
		keymaster*)
			...
			if [ -e "$rootout/$decrypt_fbe_rc" ]; then
				...
				remove_line "$decrypt_fbe_rc" "$service_name"
			fi
			;;
	esac
done
