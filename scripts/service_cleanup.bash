...
if [ "$sdkver" -lt 29 ]; then
	...
	included_blobs=($(find "$venbin" -type f \( -name "*keymaster*" -o -name "*gatekeeper*" \) | awk -F'/' '{print $NF}'))
else
	...
	included_blobs=($(find "$sysbin" -type f \( -name "*keymaster*" -o -name "*gatekeeper*" \) | awk -F'/' '{print $NF}'))
fi
echo "Blobs parsed:"
printf '%s\n' "${included_blobs[@]}"
echo " "
...
for service in ${services_not_included[@]}; do
	if [ "$sdkver" -lt 29 ]; then
		...
		if [ "$is_fbe" = "true" ]; then
			service_name+=($(grep -E "$service( |$)" "$decrypt_fbe_rc" | sed -E 's/.*service (.*) \/sbin.*/\1/'))
		fi
	else
		# android 10.0 branch
		service_name=($(grep -E "$service( |$)" "$decrypt_rc" | sed -E 's/.*service (.*) \/system.*/\1/'))
		if [ "$is_fbe" = "true" ]; then
			service_name+=($(grep -E "$service( |$)" "$decrypt_fbe_rc" | sed -E 's/.*service (.*) \/system.*/\1/'))
		fi
	fi
